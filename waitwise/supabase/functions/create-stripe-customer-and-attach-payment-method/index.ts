...
serve(async (req) => {
  ...
  try {
    ...
    if (!payment_method_id || !email || !shop_id) {
      return new Response(
        JSON.stringify({
          error: "Missing required fields: payment_method_id, email, shop_id",
        }),
        {
    ...
    const attachedPaymentMethodId = payment_method_id;
    ...
    if (customerId) {
      ...
      console.log(
        `Attached new payment method ${payment_method_id} to existing customer ${customerId}`,
      );
    } else {
      ...
      console.log(
        `Created new Stripe customer ${customerId} and attached payment method.`,
      );
    }
    ...
    if (
      ...
      console.log(
        `Shop ${shop_id} status transitioning from ${shop.subscription_status || "null"} to 'active' during card update.`,
      );
    } else {
      console.log(
        `Shop ${shop_id} subscription status remains '${shop.subscription_status}' during card update (already active/past_due).`,
      );
    ...
    if (updateError) {
      ...
      return new Response(
        JSON.stringify({
          error: `Failed to update shop record: ${updateError.message}`,
        }),
        {
          headers: { ...corsHeaders, "Content-Type": "application/json" },
          status: 500,
    ...
    return new Response(
      JSON.stringify({
        ...
        current_shop_status:
          updateShopData.subscription_status || shop.subscription_status,
      }),
      {
        headers: { ...corsHeaders, "Content-Type": "application/json" },
        status: 200,
