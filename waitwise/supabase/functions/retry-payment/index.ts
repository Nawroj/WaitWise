...
const corsHeaders = {
  ...
  "Access-Control-Allow-Headers":
    "authorization, x-client-info, apikey, content-type", // Crucial: allow x-client-info
...
Deno.serve(async (req) => {
  ...
  try {
    ...
    if (shopError || !shop || !shop.stripe_customer_id) {
      console.error(
        `[retry-payment] Error fetching shop or missing Stripe customer ID for ${shop_id}:`,
        shopError?.message || "Shop or customer ID not found",
      );
      return new Response(
        JSON.stringify({
          error: "Shop not found or Stripe customer ID missing.",
        }),
        {
          headers: { ...corsHeaders, "Content-Type": "application/json" },
          status: 404,
    ...
    console.log(
      `[retry-payment] Found Stripe Customer ID: ${stripeCustomerId}`,
    );

    // 2. Fetch the latest failed invoice for this customer from your Supabase 'invoices' table
    console.log(
      `[retry-payment] Fetching latest failed invoice for customer ${stripeCustomerId} (shop ${shop_id})`,
    );
    ...
    if (
      invoiceError ||
      !latestFailedInvoice ||
      !latestFailedInvoice.stripe_invoice_id
    ) {
      console.warn(
        `[retry-payment] No failed invoice found in Supabase for shop_id: ${shop_id}. Error:`,
        invoiceError?.message,
      );
      return new Response(
        JSON.stringify({
          error: "No outstanding failed invoice found to retry for this shop.",
        }),
        {
          headers: { ...corsHeaders, "Content-Type": "application/json" },
          status: 404,
    ...
    console.log(
      `[retry-payment] Found local failed invoice ID: ${localInvoiceId}, Stripe Invoice ID: ${stripeInvoiceId}`,
    );
    ...
    try {
      ...
      console.log(
        `[retry-payment] Retrieving invoice ${stripeInvoiceId} from Stripe.`,
      );
      stripeInvoice = await stripe.invoices.retrieve(stripeInvoiceId);
      console.log(
        `[retry-payment] Stripe Invoice retrieved. Status: ${stripeInvoice.status}, Amount: ${stripeInvoice.amount_due}`,
      );

      // Check if the invoice is already paid or open
      if (stripeInvoice.status === "paid") {
        console.log(
          `[retry-payment] Invoice ${stripeInvoiceId} already paid in Stripe. Updating DB.`,
        );
        await supabase
          .from("invoices")
          .update({
            status: "paid",
            amount_paid: latestFailedInvoice.amount_due,
          })
        ...
        await supabase
          ...
          .eq("id", shop_id);
        return new Response(
          JSON.stringify({ message: "Invoice already paid." }),
          {
            headers: { ...corsHeaders, "Content-Type": "application/json" },
            status: 200,
          },
      ...
      if (stripeInvoice.status !== "open") {
        console.warn(
          `[retry-payment] Invoice ${stripeInvoiceId} is not 'open' (current status: ${stripeInvoice.status}). Cannot retry payment.`,
        );
        return new Response(
          JSON.stringify({
            error: `Invoice status is ${stripeInvoice.status}. Cannot retry payment.`,
          }),
          {
            headers: { ...corsHeaders, "Content-Type": "application/json" },
            status: 400,
      ...
      console.error(
        `[retry-payment] Error retrieving invoice ${stripeInvoiceId} from Stripe:`,
        stripeRetrieveError.message,
      );
      return new Response(
        JSON.stringify({
          error: `Failed to retrieve invoice from Stripe: ${stripeRetrieveError.message}`,
        }),
        {
          headers: { ...corsHeaders, "Content-Type": "application/json" },
          status: 500,
    ...
    try {
      ...
      console.log(
        `[retry-payment] Attempting to pay Stripe Invoice ${stripeInvoiceId} automatically.`,
      );
      paymentResult = await stripe.invoices.pay(stripeInvoiceId, {
        off_session: true, // <-- FIXED: Changed from offSession to off_session
      });
      console.log(
        `[retry-payment] Stripe Invoice pay attempt finished. Result status: ${paymentResult.status}`,
      );
    } catch (stripePayError: any) {
      console.error(
        `[retry-payment] Error paying invoice with Stripe for ${stripeInvoiceId}:`,
        stripePayError.message,
        `Raw error: ${JSON.stringify(stripePayError.raw)}`,
      );
      ...
      console.log(
        `[retry-payment] Payment failed during pay call. Frontend error message: ${errorMessage}`,
      );
    ...
    if (paymentResult.status === "paid") {
      console.log(
        `[retry-payment] Payment result is 'paid'. Updating DB for invoice ${localInvoiceId} and shop ${shop_id}.`,
      );
      const { error: updateInvoiceError } = await supabase
        ...
        .update({
          ...
          stripe_charge_id: (paymentResult.charge as string) || null,
        })
        .eq("id", localInvoiceId);

      if (updateInvoiceError) {
        console.error(
          `[retry-payment] Error updating invoice status to paid in DB for ${localInvoiceId}:`,
          updateInvoiceError,
        );
      }
      ...
      if (updateShopError) {
        console.error(
          `[retry-payment] Error updating shop subscription status/balance in DB for ${shop_id}:`,
          updateShopError,
        );
      }
      ...
      console.log(
        `[retry-payment] Payment result is 'requires_action'. Invoice ${stripeInvoiceId} requires further action.`,
      );
      const errorMessage =
        "Payment requires further action (e.g., 3D Secure verification). Please try updating your payment method.";
      await supabase
        .from("invoices")
      ...
      console.log(
        `[retry-payment] Payment result is unexpected status: ${paymentResult.status}. Marking as failed.`,
      );
    ...
    console.error(
      `[retry-payment] Unexpected error in function execution:`,
      error.message,
      error,
    );
    return new Response(
