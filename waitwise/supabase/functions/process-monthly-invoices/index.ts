...
serve(async (req) => {
  ...
  */
  ...
  try {
    ...
    console.log(
      `Processing invoices for period (UTC): ${prevMonthStartDate.toISOString()} to ${currentMonthStartDate.toISOString()}`,
    );
    ...
    if (pricingError || !pricingTier) {
      ...
      return new Response(
        JSON.stringify({ error: "Pricing tier not found or database error." }),
        {
    ...
    const { data: shops, error: shopsError } = await supabaseClient
      .from("shops")
      .select(
        "id, name, email, stripe_customer_id, stripe_payment_method_id, account_balance, subscription_status",
      )
    ...
    if (shopsError) {
      ...
      return new Response(
        JSON.stringify({ error: "Failed to fetch shops for billing." }),
        {
          headers: { "Content-Type": "application/json" },
          status: 500,
        },
    ...
    for (const shop of shops) {
      console.log(
        `Processing shop: ${shop.name} (${shop.id}), current status: ${shop.subscription_status}`,
      );

      const currentShopBalance = shop.account_balance || 0;
      ...
      if (eventsError) {
        console.error(
          `Error fetching billable events for shop ${shop.id}:`,
          eventsError,
        );
        billingResults.push({
          shop_id: shop.id,
      ...
      if (totalAmountDue <= 0) {
        console.log(
          `Shop ${shop.id}: No new amount due for events (${eventCount}) and no outstanding balance (${currentShopBalance}). Skipping invoice creation.`,
        );
        billingResults.push({
          shop_id: shop.id,
          status: "skipped",
          message: "No amount due or balance to clear.",
        });
        continue; // Move to next shop
      }
      ...
      try {
        ...
        stripeInvoiceResponse = await stripe.invoices.create({
          ...
          metadata: {
            // IMPORTANT: Add metadata to link back to your shop
            shop_id: shop.id,
          },
        ...
        stripeInvoiceResponse = await stripe.invoices.pay(
          stripeInvoiceResponse.id,
        );

        if (stripeInvoiceResponse.status === "paid") {
          finalInvoiceStatus = "paid";
          stripeChargeId = stripeInvoiceResponse.charge
            ? (stripeInvoiceResponse.charge as Stripe.Charge).id
            : null;
          console.log(
            `Invoice ${stripeInvoiceResponse.id} for shop ${shop.id} successfully paid.`,
          );
          billingResults.push({
            shop_id: shop.id,
            status: "paid",
            stripe_invoice_id: stripeInvoiceResponse.id,
          });
        } else {
          // If status is 'open', 'draft' (after auto_advance fails), or other non-paid
          finalInvoiceStatus = "failed";
          console.warn(
            `Invoice ${stripeInvoiceResponse.id} for shop ${shop.id} payment failed or is pending. Status: ${stripeInvoiceResponse.status}`,
          );
          billingResults.push({
            shop_id: shop.id,
            status: "failed",
        ...
        finalInvoiceStatus = "failed";
        console.error(
          `Stripe API error for shop ${shop.id}:`,
          stripeError.message,
        );
        billingResults.push({
          shop_id: shop.id,
      ...
      const { data: insertedInvoice, error: insertInvoiceError } =
        await supabaseClient
          ...
          .insert({
            ...
            ).toISOString(), // Due in 7 days from today's run
          })
          .select("id")
          .single();
      ...
      if (insertInvoiceError) {
        console.error(
          `Error inserting invoice for shop ${shop.id}:`,
          insertInvoiceError,
        );
        billingResults.push({
          shop_id: shop.id,
          status: "error",
          message: `Failed to record invoice in DB: ${insertInvoiceError.message}`,
        });
        continue;
      }
      ...
      if (billableEvents.length > 0 && newInvoiceId) {
        // Ensure there are events and an invoice was created
        ...
        if (updateEventsError) {
          console.error(
            `Error updating billable_events invoice_id for shop ${shop.id}:`,
            updateEventsError,
          );
          billingResults.push({
            shop_id: shop.id,
            status: "error",
            message: `Failed to update billable events invoice_id: ${updateEventsError.message}`,
          });
        }
      }
    ...
    return new Response(
      JSON.stringify({
        message: "Monthly invoicing process completed.",
    ...
    console.error(
      "An unhandled error occurred in process-monthly-invoices:",
      error.message,
      error,
    );
    return new Response(
