...
Deno.serve(async (req) => {
  ...
  if (!sig || !STRIPE_WEBHOOK_SECRET) {
    console.error(
      "Missing Stripe-Signature header or STRIPE_WEBHOOK_SECRET environment variable.",
    );
    return new Response(
  ...
  console.log(
    `Received Stripe event type: ${event.type} for object ID: ${event.data.object.id}`,
  );
  ...
  try {
    switch (event.type) {
      case "invoice.payment_succeeded": {
        ...
        console.log(
          `Processing invoice.payment_succeeded for invoice: ${invoice.id}, shopId from metadata: ${shopId}`,
        );

        if (!shopId) {
          console.warn(
            `invoice.payment_succeeded: shop_id not found in invoice metadata. Attempting to lookup via customer ID: ${customerId}`,
          );
          ...
          if (findShopError || !shops) {
            console.error(
              "Could not find shop by customer ID:",
              customerId,
              findShopError?.message,
            );
            return new Response(
              JSON.stringify({
                received: true,
                error: "Shop not found for customer",
              }),
              { status: 200 },
            ); // Still acknowledge
          }
        ...
        const { error: invoiceUpdateError } = await supabase
          ...
          .update({
            ...
            stripe_charge_id: invoice.charge
              ...
              : null, // Ensure string type
          })
          .eq("stripe_invoice_id", invoice.id);

        if (invoiceUpdateError) {
          console.error(
            "Error updating invoice to paid in DB:",
            invoiceUpdateError,
          );
        } else {
        ...
        if (shopUpdateError) {
          console.error(
            "Error updating shop status to active/balance in DB:",
            shopUpdateError,
          );
        } else {
          console.log(
            `Shop ${shopId} status set to 'active' and balance reset.`,
          );
        }
        break;
      }
      case "invoice.payment_failed": {
        ...
        console.log(
          `Processing invoice.payment_failed for invoice: ${invoice.id}, shopId from metadata: ${shopId}, Reason: ${failedReason}`,
        );

        if (!shopId) {
          console.warn(
            `invoice.payment_failed: shop_id not found in invoice metadata. Attempting to lookup via customer ID: ${customerId}`,
          );
          const { data: shops, error: findShopError } = await supabase
            .from("shops")
            .select("id")
        ...
        if (invoiceUpdateError) {
          console.error(
            "Error updating invoice to failed in DB:",
            invoiceUpdateError,
          );
        } else {
        ...
        if (shopUpdateError) {
          console.error(
            "Error updating shop status to past_due/balance in DB:",
            shopUpdateError,
          );
        } else {
          console.log(
            `Shop ${shopId} status set to 'past_due' and balance updated to ${invoice.amount_due}.`,
          );
        }
        break;
      }
      case "customer.subscription.updated": {
        ...
        console.log(
          `Processing customer.subscription.updated for customer: ${customerId}, new status: ${newStatus}`,
        );
        ...
        if (shopError || !shopData) {
          console.error(
            "customer.subscription.updated: Shop not found for customer:",
            customerId,
            shopError?.message,
          );
          return new Response(
        ...
        if (updateError) {
          console.error(
            "Error updating shop subscription status via webhook (customer.subscription.updated):",
            updateError,
          );
        } else {
          console.log(
            `Shop ${shopData.id} subscription status set to '${newStatus}' via customer.subscription.updated webhook.`,
          );
        }
        break;
      }
